...
class SoundManager {
  ...
  startMusic() {
    ...
    // Fade music in.
    this.musicGain.gain.setTargetAtTime(0.22, this.ctx.currentTime, 0.5)
    this.musicStep = 0
    const step = () => {
      const seq = SoundManager.MUSIC_SEQUENCE
    ...
    step()
    this.musicTimer = setInterval(step, SoundManager.MUSIC_STEP_MS)
  }
